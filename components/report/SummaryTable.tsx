...
// 現行クライアント要望（2026-05-06）に合わせて10行に絞る。
// 削除済: 事前CC数/率, 無断CC数, 新患数, 初回メンテ移行数, メンテナンス数, メンテナンス率
const SUMMARY_ROWS: RowDef[] = [
  ...
  { id: 'reserved_count',       label: '予約取得数',         unit: '名', format: 'int' },
  { id: 'unreserved_count',     label: '未予約患者数',       unit: '名', format: 'int' },
  { id: 'reserved_rate',        label: '予約取得率',         unit: '%',  format: 'percent' },
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          <thead className="bg-slate-50 text-[10px] uppercase font-black text-slate-400 tracking-widest">
            <tr>
              <th className="text-left px-4 py-3 min-w-[180px] whitespace-nowrap sticky left-0 bg-slate-50 z-10">KPI</th>
              {rows.map(r => (
          ...
          <tbody>
            {SUMMARY_ROWS.map((row, idx) => (
              <tr key={row.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'}>
                <td className={`text-left px-4 py-2.5 font-black text-slate-700 whitespace-nowrap sticky left-0 z-10 ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'}`}>
                  {row.label}
