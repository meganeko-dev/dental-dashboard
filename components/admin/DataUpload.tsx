...
//         try {
//           // 1. ファイル名を正規化（Mac特有の濁点分離問題を解決）してパターンとIDを抽出
...
//             clinicId = nameParts[0];
//             filePattern = "status";
//           } else if (fileName.includes("ステージ日別状況")) {
//             // "2025_866_" のように年が先頭にある場合は2番目をIDとする
//             clinicId = nameParts[0].length === 4 && !isNaN(Number(nameParts[0])) ? nameParts[1] : nameParts[0];
//             filePattern = "stage";
//           } else {
//             throw new Error('未対応のファイル名です。「月ごとのStats」「医院状況」「ステージ日別状況」のいずれかが含まれている必要があります。');
//           }
...
//             transformed = DataImporter.transformStats(rawData, clinicName, targetCorpId);
//           } else if (filePattern === 'status') {
//             transformed = DataImporter.transformStatus(rawData, clinicName, targetCorpId);
//           } else if (filePattern === 'stage') {
//             transformed = DataImporter.transformStage(rawData, clinicName, targetCorpId);
//           }
...
//               onConflict: 'corporation_id, clinic_name, staff_name, year, month, segment, kpi_name, is_target, treatment_type, staff_role'
//             });
