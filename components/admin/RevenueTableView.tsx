...
// 表示期間の起点 (2026-06-17 設定: 2025/1 固定)
const TABLE_START_YEAR = 2025
const TABLE_START_MONTH = 1
...
const ymKey = (y: number, m: number): MonthKey => `${y}-${m}`

type RowData = {
  name: string
  isClinicTotal: boolean
  byMonth: { [key: MonthKey]: { insurance: number; private: number; product: number } }
}
...
export function RevenueTableView({ corpId }: { corpId: string }) {
  ...
  const months = useMemo(
    () => buildMonthRange(TABLE_START_YEAR, TABLE_START_MONTH, currentPeriod.year, currentPeriod.month),
    [currentPeriod],
  )
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const { data } = await supabase
        .from('flexible_kpis')
        .select('segment, staff_name, kpi_name, value, year, month')
        .eq('corporation_id', corpId)
        .eq('clinic_name', clinicName)
        .gte('year', TABLE_START_YEAR)
        .eq('is_target', false)
        .in('kpi_name', [
  ...
  const rows: RowData[] = useMemo(() => {
    const monthKeySet = new Set(months.map(({ year, month }) => ymKey(year, month)))
    const emptyMonthMap = () => {
      const m: RowData['byMonth'] = {}
      for (const { year, month } of months) m[ymKey(year, month)] = { insurance: 0, private: 0, product: 0 }
      return m
    }
    ...
    for (const r of rawRows) {
      ...
      if (r.segment === 'clinic') {
        if (CLINIC_INSURANCE_KPIS.includes(r.kpi_name))      clinicRow.byMonth[key].insurance += v
        else if (r.kpi_name === CLINIC_PRIVATE_KPI)          clinicRow.byMonth[key].private   += v
        else if (r.kpi_name === CLINIC_PRODUCT_KPI)          clinicRow.byMonth[key].product   += v
        ...
        if (r.kpi_name === STAFF_INSURANCE_KPI)      row.byMonth[key].insurance += v
        else if (r.kpi_name === STAFF_PRIVATE_KPI)   row.byMonth[key].private   += v
        else if (r.kpi_name === STAFF_PRODUCT_KPI)   row.byMonth[key].product   += v
      }
    }
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="bg-white rounded-3xl border shadow-sm p-6 flex flex-wrap items-center gap-4">
        <div className="flex flex-col gap-1">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">表示期間</label>
          <div className="bg-slate-50 rounded-2xl px-4 py-2.5 text-sm font-black text-slate-700 w-fit">
            {TABLE_START_YEAR}/{TABLE_START_MONTH} 〜 {currentPeriod.year}/{currentPeriod.month}
          </div>
        </div>
        {clinics.length > 1 && (
          <div className="flex flex-col gap-1">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-[11px]">
              <thead className="bg-slate-50 text-[10px] text-slate-400 font-black uppercase sticky top-0">
                <tr>
                  ...
                  {months.map(({ year, month }) => (
                    <th key={ymKey(year, month)} className="px-1.5 py-2.5 text-right min-w-[72px]">{year}/{month}</th>
                  ))}
              ...
              <tbody className="divide-y border-t">
                {rows.length === 0 ? (
                  <tr>
                    <td colSpan={months.length + 3} className="p-10 text-center text-slate-400 font-bold italic">データがありません</td>
                  </tr>
                ) : (
                  rows.flatMap((row, rowIdx) => (
                    SUB_CATEGORIES.map((sub, subIdx) => {
                      const monthValues = months.map(({ year, month }) => row.byMonth[ymKey(year, month)][sub.key])
